    ...
    for index in file_list:
        ...
        with open(os.path.join(os.path.abspath('..'), 'feature_1.0', str(index) + '.txt'), 'r', encoding='UTF-8') as fread:
            ...
            for item in res:
                ...
                KNN_tmp = []
                feature_list = [6,1,2,3,4,5,7,8,9,10]
                #feature_list = [1,7,8,6,11,12,2,3,4,5,9,10]
                ...
                KNN_target.append(int(res[item][12]))

    target = list(flatten_target)
    target.sort(reverse=True)

    cut_level = int(len(target)*0.8)
    print('cut_level')
    print(target[cut_level])

    level1 = int(len(target)*0.001)
    level2 = int(len(target)*0.003)
    level3 = int(len(target)*0.043)
    level4 = int(len(target)*0.243)
    print(str(target[0]) + ' ' + str(target[cut_level]))
    print(str(target[level1]) + ' ' + str(target[level2]) + ' ' + str(target[level3]) + ' ' + str(target[level4]))

    flatten_target_copy = flatten_target.copy()
    flatten_target = []
    flatten_data_copy = flatten_data.copy()
    flatten_data = []
    KNN_data_copy = KNN_data.copy()
    KNN_data = []
    KNN_target_copy = KNN_target.copy()
    KNN_target = []

    for i in range(len(flatten_target_copy)):
        if flatten_target_copy[i] > target[cut_level]:

            KNN_data.append(KNN_data_copy[i])
            KNN_target.append(KNN_target_copy[i])
            flatten_data.append(flatten_data_copy[i])

            if flatten_target_copy[i] < target[level4]:
                flatten_target.append(0)
            elif flatten_target_copy[i] < target[level3]:
                flatten_target.append(1)
            elif flatten_target_copy[i] < target[level2]:
                flatten_target.append(2)
            elif flatten_target_copy[i] < target[level1]:
                flatten_target.append(3)
            else:
                flatten_target.append(4)

    print('length of flatten data')
